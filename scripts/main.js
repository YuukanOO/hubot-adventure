...
//         characterMsg.reply(db.racesAttachments(`Bien le bonjour *${character.name}* !
// A quelle race appartient tu ?`));
...
//           raceMsg.reply(db.classesAttachments(`Un fier *${character.race}* donc ! Nous avançons, quelle classe maintenant ?`));

//           classes.forEach(c => dialog.addChoice(new RegExp(c, 'i'), function (classMsg) {
//             character.class = c;
//             character.health = db.classes[c].health;
//             let availableStats = [];
...
//                 character.computeModifiers(db.modifiers);
